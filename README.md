...
## Database setup (existing PostgreSQL container)

This project is configured to use an existing local PostgreSQL instance:

```bash
DATABASE_URL=postgresql://postgres:password@localhost:5432/agent_os
```

Create the database if it does not exist (container: `vfri_postgres`):
